...
const loadConnection = async (supabaseUrl: string, serviceRoleKey: string) => {
  const response = await fetch(
    `${supabaseUrl}/rest/v1/instagram_dashboard_connection?select=access_token&id=eq.owner&limit=1`,
    { headers: serviceHeaders(serviceRoleKey) }
  );
  const body = await response.json();
  if (!response.ok) throw new Error(body.message || "Could not load Instagram connection.");
  return body[0] as { access_token: string } | undefined;
};
...
const accountViewsForDay = async (profileId: string, accessToken: string, dayStart: Date) => {
  const nextDay = new Date(dayStart.getTime() + DAY_MS);
  const insights = await graphGet(`${profileId}/insights`, accessToken, {
...
const mediaInsights = async (mediaId: string, token: string) => {
  ...
  try {
    const insights = await graphGet(`${mediaId}/insights`, token, {
    ...
    await Promise.all(mediaInsightMetrics.map(async (metric) => {
      try {
        const insights = await graphGet(`${mediaId}/insights`, token, { metric });
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const media = await graphGet("me/media", accessToken, {
    ...
    const mediaWithInsights = await Promise.all(
      (media.data ?? []).map(async (item: any) => {
        try {
          const values = await mediaInsights(item.id, accessToken);
          const views = values.views ?? item.total_views_count ?? 0;
    ...
    const dailyViews = await inBatches(insightDays, 5, async (day) => {
      try {
        return {
          day: isoDate(day),
          views: await accountViewsForDay(profile.id, accessToken, day)
        };
      } catch (err) {
    ...
    if (!userInsightsTimeline.length && userInsightsErrors.length) {
      throw new Error(`Instagram daily account views failed: ${userInsightsErrors[0].error}`);
    }
