...
/**
 * Speechify narration player (simba-3.2 / geffen_32 by default).
...
export const createSpeechifyPlayer = ({
  ...
  const run = async () => {
    const { data, error } = await supabase.functions.invoke("edge-tts", {
      body: { rate: speechRate, text: text.slice(0, 2000), voice }
    });
    ...
    const words = (
      data && typeof data === "object" && Array.isArray((data as { words?: unknown }).words)
        ? ((data as { words: Array<{ text: string; start: number; end: number }> }).words)
        : []
    ).filter(
      (word) =>
        word &&
    ...
    if (!timedWords.length && wordRanges.length) {
      wordRanges.forEach((range, index) => {
        timedWords.push({ ...range, startsAt: (index * 0.31) / speechRate });
      });
    }
